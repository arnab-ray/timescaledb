		...
		case DIMENSION_TYPE_OPEN:
			{
				...
				return (DimensionRestrictInfo *) new;
			}
...
{
	int16		capacity;
	int			num_added_restrictions; /* number of base restrictions
										 * successfully added */
	DimensionRestrictInfo *diminson_restriction[FLEXIBLE_ARRAY_MEMBER]; /* key is attrOffset
																		 * (AttrNo-1)  */
...
{
	HypertableRestrictInfo *res = palloc0(sizeof(HypertableRestrictInfo) + sizeof(DimensionRestrictInfo *) * rel->max_attr);
	int			i;

	res->capacity = rel->max_attr;
...
{
	Assert(attno > 0 && AttrNumberGetAttrOffset(attno) < hri->capacity);
	return hri->diminson_restriction[AttrNumberGetAttrOffset(attno)];
}
...
{
	return hri->num_added_restrictions > 0;
}
...
hypertable_restrict_info_get_chunk_oids(HypertableRestrictInfo *hri, Hypertable *ht, LOCKMODE lockmode)
{
	...
	for (i = 0; i < hri->capacity; i++)
	{
		DimensionRestrictInfo *dri = hri->diminson_restriction[i];

		if (dri != NULL)
		{
			DimensionVec *dv = dimension_restrict_info_slices(dri);

			Assert(dv->num_slices >= 0);

			/*
			 * If there are no matching slices in any dimension, the result
			 * will be empty
			 */
			if (dv->num_slices == 0)
				return NIL;
	...
	oids = chunk_find_all_oids(ht->space, dimension_vecs, lockmode);
	return oids;
}
